...
LOG_FN_ENTER();
    ...
    if (yed_plugin_make_ft(self, "Make") == FT_ERR_TAKEN) {
        yed_cerr("lang/make: unable to create file type name");
        LOG_EXIT();
        return 1;
    }
    ...
    yed_set_var("Make-comment-style", "#");
...
void maybe_change_ft(yed_buffer *buff) {
    ...
    if (buff->ft != FT_UNKNOWN) { return; }
    if (buff->path == NULL)     { return; }

    if ((base = get_path_basename(buff->path)) != NULL) {
        if (strcmp(base, "Makefile")                        == 0
        ||  strcmp(base, "makefile")                        == 0
        ||  strcmp(base, "GNUMakefile")                     == 0
        ||  strcmp(base, "Makefile.include")                == 0
        ||  strncmp(base, "Makefile.", strlen("Makefile.")) == 0
        ||  strncmp(base, "makefile.", strlen("makefile.")) == 0) {

            yed_buffer_set_ft(buff, yed_get_ft("Make"));
            return;
        }
    }
